...
function v = fd_am_put_sor(r,q,s,x,sig,t,dt,dx,w)
    ...
    for j=N:-1:1
        ...
        while(flag)
            ...
            u(1) = max((1-w)*prev_u(1)+w*u_gs(1),max(x-exp(dx*(I-1)),0)-prev_u(1)); 
            for i=2:I-2
                u_gs(i) = 1/b * (-a*u_gs(i-1)-c*prev_u(i+1)+Rhs(i));
                u(i) = max((1-w)*prev_u(i)+w*u_gs(i),max(0,x-exp(dx*(I-i)))-prev_u(i));
            end
            
            u_gs(I-1) = 1/b*(-a*u_gs(I-2)-Rhs(I-1));
            u(I-1) = max((1-w)*prev_u(I-1)+w*u_gs(I-1),max(x-exp(dx*(1)),0)-prev_u(I-1));
            
            if(abs(u'-prev_u)<0.00001)
                flag = 0;
            else
                prev_u = u';
            end
